  ...
  public:
    ...
    static auto match_path(std::string_view pattern, std::string_view path) -> bool
    {
        auto split_path = [](std::string_view str) -> std::vector<std::string_view>
        {
            return str | std::views::split('/') |
                   std::views::transform(
                       [](auto &&range)
                       {
                           return std::string_view{range.begin(), static_cast<size_t>(std::distance(
                                                                      range.begin(), range.end()))};
                       }) |
                   std::views::filter([](auto &&inside_str) { return !inside_str.empty(); }) |
                   std::ranges::to<std::vector>();
        };

        auto parse_query =
            [](std::string_view query) -> std::unordered_map<std::string_view, std::string_view>
        {
            std::unordered_map<std::string_view, std::string_view> params;
            auto                                                   key_value_pairs =
                query | std::views::split('&') |
                std::views::transform(
                    [](auto &&range)
                    {
            ...
            std::ranges::for_each(
                ...
                [&](auto &&key_value_pair)
                {
                    auto key_value = key_value_pair | std::views::split('=') |
                                     std::views::transform(
                                         [](auto &&range)
                                         {
        ...
        if (pattern_question_pos != std::string_view::npos)
        {
            ...
            std::string_view const path_query    = (path_question_pos != std::string_view::npos)
                                                       ? path.substr(path_question_pos + 1)
                                                       : "";
            if (path_query.empty())
            {
                return false;
