...
class HttpResponse
{
  ...
  public:
    ...
    void set_body(std::string text) { m_body = std::move(text); }
    void set_body_json(const Json::Value &json_obj);

    void set_status(int status_code);

    void set_version(std::string_view http_ver) { m_http_version = http_ver; }

    void set_custom_message(std::string msg) { m_status_message = std::move(msg); }
};
